...
export default function DatabaseTab({ isDarkMode }: DatabaseTabProps) {
  ...
  const filteredFolders = folders.filter(f => {
    ...
    const matchObj = filterObject === 'Semua Objek' || f.objectType === filterObject;
  ...
  const saveForm = () => {
    ...
    if (formMode === 'create') {
      const newFolder = { ...formData, id: Math.random().toString(36).substr(2, 9), date: new Date().toISOString().split('T')[0], imageCount: 0 };
      setFolders([newFolder, ...folders]);
    } else {
      setFolders(folders.map(f => f.id === formData.id ? { ...f, ...formData } : f));
    }
    setIsFormOpen(false);
  };
          ...
          <div className="flex items-center justify-between w-full flex-wrap gap-2">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className={`text-xs ${theme.textMuted}`}>{activeFolder?.imageCount} Gambar • Objek: {activeFolder?.objectType}</p>
              </div>
            </div>
              ...
              <div 
                ...
                <div className="flex items-start gap-4 cursor-pointer mb-4" onClick={() => { setActiveFolder(folder); setViewMode('images'); }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className={`text-xs font-bold text-blue-500 mb-1`}>{folder.objectType}</p>
                ...
                <div className="flex items-center justify-between">
                  <span className={`text-xs font-bold px-2 py-1 rounded bg-black/5 ${theme.textMuted}`}>{folder.imageCount} Gambar</span>
                  <div className="flex gap-2">
